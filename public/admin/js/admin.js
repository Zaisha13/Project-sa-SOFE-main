...
function initializeAdminDashboard() {
    ...
    // Calculate and display order stats
    function updateOrderStats() {
        try {
            ...
            // For now, we'll distribute orders between branches randomly
            // In production, this should come from order metadata
            const smSjdmOrders = todayOrders.filter((order, index) => index % 2 === 0);
            const smFairviewOrders = todayOrders.filter((order, index) => index % 2 === 1);

            if (smSjdmCount) smSjdmCount.textContent = smSjdmOrders.length;
            if (smFairviewCount) smFairviewCount.textContent = smFairviewOrders.length;
                        ...
                        <table style="width:100%;border-collapse:collapse;font-size:13px;">
                            <thead>
                                <tr>
                                    ...
                                    <th style="padding:12px 10px;border:1px solid #f0f0f0;background:transparent;color:var(--muted);font-weight:700;font-size:12px;text-transform:uppercase;">Customer</th>
                                    ...
                                    <th style="padding:12px 10px;border:1px solid #f0f0f0;background:transparent;color:var(--muted);font-weight:700;font-size:12px;text-transform:uppercase;">Total</th>
                            ...
                            <tbody>
                                ${orders.map(order => `
                                    <tr style="border-bottom:1px solid #f0f0f0;">
                                        ...
                                        <td style="padding:12px 10px;border:1px solid #f0f0f0;color:#2a2a2a;">${(order.items || []).length} item(s)</td>
                                        ...
                                        <td style="padding:12px 10px;border:1px solid #f0f0f0;color:#2a2a2a;">
                                          <span style="
                                            background: ${String(order.status||'').toLowerCase()==='completed' ? '#10b981' : (String(order.status||'').toLowerCase()==='cancelled' ? '#ef4444' : '#f59e0b')};
                                            color: #fff; padding: 6px 10px; border-radius: 999px; font-size: 12px; font-weight: 700; letter-spacing:.2px;">
    ...
    if (smSjdmCard) {
        smSjdmCard.addEventListener('click', () => {
            const orders = JSON.parse(localStorage.getItem('jessie_orders') || '[]');
            const today = new Date().toDateString();
            const todayOrders = orders.filter(order => {
                const orderDate = new Date(order.timestamp || new Date(order.date)).toDateString();
    ...
    if (smFairviewCard) {
        smFairviewCard.addEventListener('click', () => {
            const orders = JSON.parse(localStorage.getItem('jessie_orders') || '[]');
            const today = new Date().toDateString();
            const todayOrders = orders.filter(order => {
                const orderDate = new Date(order.timestamp || new Date(order.date)).toDateString();
    ...
    function getCredentialsFor(target){
        if (target === 'cashier_fairview') {
            return JSON.parse(localStorage.getItem(CASHIER_FAIRVIEW_KEY) || '{"email":"cashier.fairview@jessiecane.com","password":"cashier123"}');
        } else if (target === 'cashier_sjdm') {
            return JSON.parse(localStorage.getItem(CASHIER_SJDM_KEY) || '{"email":"cashier.sjdm@jessiecane.com","password":"cashier123"}');
        }
        return JSON.parse(localStorage.getItem(ADMIN_CREDENTIALS_KEY) || '{"email":"admin@jessiecane.com","password":"admin123"}');
    }
    ...
    if (changeEmailForm) {
        changeEmailForm.addEventListener('submit', (e) => {
            ...
            showToast('success','Updated','Email updated successfully!');
    ...
    function initializeDefaultData() {
        ...
        if (!localStorage.getItem(ADMIN_CREDENTIALS_KEY)) {
            localStorage.setItem(ADMIN_CREDENTIALS_KEY, JSON.stringify({ email: 'admin@jessiecane.com', password: 'admin123' }));
        }
        if (!localStorage.getItem(CASHIER_FAIRVIEW_KEY)) {
            localStorage.setItem(CASHIER_FAIRVIEW_KEY, JSON.stringify({ email: 'cashier.fairview@jessiecane.com', password: 'cashier123' }));
        }
        if (!localStorage.getItem(CASHIER_SJDM_KEY)) {
            localStorage.setItem(CASHIER_SJDM_KEY, JSON.stringify({ email: 'cashier.sjdm@jessiecane.com', password: 'cashier123' }));
        }
    ...
    function setupSalesReportTabs(){
        ...
        tabButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Lazy render per tab
                if (target === 'tab-daily') renderDailySales();
                if (target === 'tab-weekly') renderWeekly();
                if (target === 'tab-monthly') renderMonthly();
                if (target === 'tab-walkin') renderWalkin();
                if (target === 'tab-online') renderOnline();
                if (target === 'tab-best-sellers') renderBestSellers();
            });
        });
            ...
            if (window.flatpickr) {
                try {
                    window.flatpickr(dailyDateInput, {
                        ...
                        allowInput: true,
                        onChange: function(){ renderDailySales(); },
                        disableMobile: true
                    });
                } catch(_) {}
            }
    ...
    function renderDailySales(){
        try {
            const dailyDateInput = document.getElementById('daily-date-input');
            const selected = dailyDateInput && dailyDateInput.value ? new Date(dailyDateInput.value + 'T00:00:00') : new Date();
            const orders = getAllOrders().filter(isCompleted);
            const todays = orders.filter(o => {
                const d = orderDate(o); return d && sameDay(d, selected);
            });
            ...
            const tSJDM = document.getElementById('daily-total-sjdm'); if (tSJDM) tSJDM.textContent = `Total: ${toPhp(sumTotals(sjdm))}`;
            const tFair = document.getElementById('daily-total-fairview'); if (tFair) tFair.textContent = `Total: ${toPhp(sumTotals(fair))}`;
            const tCons = document.getElementById('daily-total-consolidated'); if (tCons) tCons.textContent = `All Branches Total: ${toPhp(sumTotals(todays))}`;
    ...
    function renderWeekly(){
        try {
            ...
            const week = orders.filter(o => { const d = orderDate(o); return inRange(d, wStart, wEnd); });
            ...
            const wSJ = document.getElementById('weekly-total-sjdm'); if (wSJ) wSJ.textContent = `Total: ${toPhp(sumTotals(weekSJDM))}`;
            const wF = document.getElementById('weekly-total-fairview'); if (wF) wF.textContent = `Total: ${toPhp(sumTotals(weekFair))}`;
            const wC = document.getElementById('weekly-total-consolidated'); if (wC) wC.textContent = `All Branches Total: ${toPhp(sumTotals(week))}`;
    ...
    function renderMonthly(){
        try {
            ...
            const orders = getAllOrders().filter(isCompleted);
            const month = orders.filter(o => { const d = orderDate(o); return inRange(d, mStart, mEnd); });
            ...
            const mSJ = document.getElementById('monthly-total-sjdm'); if (mSJ) mSJ.textContent = `Total: ${toPhp(sumTotals(monthSJDM))}`;
            const mF = document.getElementById('monthly-total-fairview'); if (mF) mF.textContent = `Total: ${toPhp(sumTotals(monthFair))}`;
            const mC = document.getElementById('monthly-total-consolidated'); if (mC) mC.textContent = `All Branches Total: ${toPhp(sumTotals(month))}`;
