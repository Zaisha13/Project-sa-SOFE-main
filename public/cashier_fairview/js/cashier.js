...
function updateOrderStatus(orderId, newStatus) {
    try {
        ...
        const orderIndex = orders.findIndex(o => o.id === orderId && branchesMatch(o.branch, cashierBranch));
        
        if (orderIndex === -1) {
            alert('Order not found or does not belong to your branch');
            return;
        }
...
function viewDetailedReport() {
    try {
        ...
        todayOrders.forEach(order => {
            ...
            // Count sales strictly from completed orders only
            if (orderStatus === 'completed') {
                completedSales += orderTotal;
                totalSales += orderTotal;
            } else if (orderStatus === 'approved') {
                approvedSales += 0; // keep row, zero value
            } else if (orderStatus === 'pending') {
                pendingSales += 0; // keep row, zero value
            } else if (orderStatus === 'out for delivery') {
                outForDeliverySales += 0; // keep row, zero value
            } else if (orderStatus === 'cancelled') {
                cancelledSales += 0; // keep row, zero value
            }
        });
